 ...
 *   GITHUB_TOKEN=...  PR_NUMBER=123  ACTION=comment_failure|approve  bun run scripts/check_and_comment.ts
 */
// @ts-nocheck
...
async function commentFailure() {
	...
	const reasonMessages: Record<string, string> = {
		network: [
			'> ⚠️ This may be a transient npm registry outage.',
			'> Please re-run the workflow before amending your PR.',
		].join('\n'),
		not_found: [
			'> ❌ The package name does not exist on the npm registry.',
		...
		execution_error: [
			...
			'> Please verify the package works with `bunx --package <package> <binary> --help`.',
		].join('\n'),
	};
...
async function approve() {
	const res = await fetch(`${base}/pulls/${github.issueNumber}/reviews`, {
		...
		body: JSON.stringify({
			event: 'APPROVE',
			body: '✅ All proposed tools validated successfully via `bunx`. Ready for merge.',
		}),
	});
